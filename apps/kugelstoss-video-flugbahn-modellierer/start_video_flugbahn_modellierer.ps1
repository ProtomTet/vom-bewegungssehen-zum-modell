...
& $python -c "import streamlit, streamlit_image_coordinates, cv2, imageio_ffmpeg" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "Installiere fehlende Pakete ..." -ForegroundColor Cyan
    & $python -m pip install -r requirements.txt
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Installation fehlgeschlagen. Bitte Internetverbindung und Python-Installation prüfen." -ForegroundColor Red
